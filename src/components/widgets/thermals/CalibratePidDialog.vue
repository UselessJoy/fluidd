<template>
  ...
  >
    <v-card>
      ...
      <v-card-title>
        {{ $tc('app.pid_calibrate.setting.temps') }}
      </v-card-title>
      <v-card-text class="overflow-y-auto">
        <template v-for="(temperature, index) in temperatures">
          ...
          <v-divider :key="`temperature-${index}`" />
        </template>
      </v-card-text>
...
export default class CalibratePidDialog extends Mixins(StateMixin) {
  ...
  changeHeater (value: string) {
    ...
    const heaterPresets = this.presets.find(preset => { return preset.name === this.currentHeater })
    if (heaterPresets) {
      for (const key in heaterPresets.values) {
